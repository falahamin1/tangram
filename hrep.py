...
def train_h_rep(episodes=1000,
                checkpoint_dir='checkpoints/hrep',
    ...
    model = DeepSetActorCritic(input_dim=15, num_pieces=6, num_actions=24)
    optimizer = optim.Adam(model.parameters(), lr=HP["lr"])
    buffer = PPOBuffer(size=HP["steps_per_rollout"], state_shape=(6, 15))
    ...
    for ep in range(start_ep, episodes):
        ...
        for t in range(HP["steps_per_rollout"]):
            raw_h_rep = torch.tensor(obs['h_rep'], dtype=torch.float32)
            state_flat = raw_h_rep.view(6, 15)
            ...
            if done:
                ...
                raw_h_next = torch.tensor(obs['h_rep'], dtype=torch.float32)
                state_next_input = raw_h_next.view(6, 15).unsqueeze(0)
                with torch.no_grad():
                    _, last_val = model(state_next_input)
        ...
        if len(reward_history) >= HP["moving_avg_window"]:
            ...
            if current_moving_avg > best_moving_avg:
                ...
                print(f"*** NEW BEST H-REP MODEL (Avg: {best_moving_avg:.2f}) at Ep {ep} ***")
        ...
        if ep % 100 == 0:
            ...
            print(f"H-Rep Ep {ep} | Recent Avg: {avg:.2f} | Best Avg: {best_moving_avg:.2f}")
    ...
    best_model = DeepSetActorCritic(input_dim=15, num_pieces=6, num_actions=24)

"""
Evaluate saved tangram policies — one greedy episode per method,
...
def load_model(method, checkpoint_dir):
    if method == 'hrep':
        ...
        model = DeepSetActorCritic(input_dim=8, num_pieces=6, num_actions=24)
...
def get_action(model, method, obs, mask):
    with torch.no_grad():
        ...
        if method == 'hrep':
            state = torch.tensor(obs['h_rep'], dtype=torch.float32).view(1, 6, 15)
            logits, _ = model(state)
        elif method == 'vrep':
            state = torch.tensor(obs['v_rep'], dtype=torch.float32).view(1, 6, 8)
            logits, _ = model(state)
        elif method == 'gnn':
...
def evaluate_method(method, checkpoint_dir):
    ...
    for step in range(1, env.max_steps + 1):
        ...
        print(f"  step {step:03d}: action={action:3d}  reward={reward:+.3f}"
              f"  total={total_reward:+.3f}  pieces={completion*6:.0f}/6")
...
def main():
    ap = argparse.ArgumentParser(description='Evaluate saved tangram policies')

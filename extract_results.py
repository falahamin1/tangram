"""
Read-only extractor: walks existing tangram-git artifacts on disk
...
Two metrics-dict formats can be present:
  ...
  - Old format (seeds 0-2, trained before that upgrade): metrics only has a
    ...
    only, no training) in both eval modes.
...
from x_example import SymbolicTangramGym, FLAT_POSE_DIM, GRID_CHANNELS
...
NUM_PIECES, H_DIM, V_DIM, NUM_ACTIONS = 6, 15, 8, 24
...
def main():
    ...
    for path in sorted(glob.glob(os.path.join(POLICY_DIR, '*.pth'))):
        ...
        if 'solve_rate_stochastic' in metrics:
            # New format: native dual-mode + entropy, no reconstruction needed.
            per_mode = {
            ...
            # Old format (seeds 0-2): reconstruct both modes via frozen-weight rollout.
    ...
    # CNN never produced a policies/cnn_seed*.pth for any seed (checkpoints
    # cap out at ep~31500/30500/33000 of 40000, no cnn_final.pth) -- explicit
    # missing rows rather than silently absent from the CSV.
    for seed in (0, 1, 2):
        if not any(r['method'] == 'cnn' and r['seed'] == seed for r in rows):
            for mode in ('greedy', 'stochastic'):
                rows.append({
                    'benchmark': 'tangram', 'tier': 'hard', 'method': 'cnn', 'seed': seed,
                    'instance_id': 'hard_fixed', 'eval_mode': mode, 'solved': '', 'steps': '',
                    'optimal_steps': '', 'rollouts_to_solve': '', 'final_entropy': '', 'provenance': 'missing',
                })
    ...
    missing = sum(1 for r in rows if r['provenance'] == 'missing')
    print(f"Wrote {len(rows)} rows to {out_path}")
    print(f"provenance: trusted={trusted}  unverified={unverified}  missing={missing}")

...
def train_graph_rep(episodes=1000,
    ...
    model = GNNActorCritic(node_dim=3, hidden_dim=128, num_actions=24)
    optimizer = optim.Adam(model.parameters(), lr=HP["lr"])

    h_buffer   = PPOBuffer(size=HP["steps_per_rollout"], state_shape=(6, 5, 3))
    adj_buffer = PPOBuffer(size=HP["steps_per_rollout"], state_shape=(6, 5, 5))
    ...
    for ep in range(start_ep, episodes):
        ...
        for t in range(HP["steps_per_rollout"]):
            ...
            h_buffer.store(h_rep.squeeze(0),   action, reward, value.item(), log_prob.item())
        ...
        if len(reward_history) >= HP["moving_avg_window"]:
            ...
            if current_moving_avg > best_moving_avg:
                ...
                print(f"*** NEW BEST GRAPH MODEL (Avg: {best_moving_avg:.2f}) at Ep {ep} ***")
        ...
        for _ in range(HP["ppo_epochs"]):
            ...
            for start in range(0, HP["steps_per_rollout"], HP["batch_size"]):
                ...
                mb_h            = data_h['states'][mb_idx]
                mb_adj          = data_adj['states'][mb_idx]
                mb_actions      = data_h['actions'][mb_idx]
                mb_adv          = data_h['advantages'][mb_idx]
                mb_ret          = data_h['returns'][mb_idx]
                mb_old_logprobs = data_h['log_probs'][mb_idx]
        ...
        if ep % 100 == 0:
            ...
            print(f"Graph-Rep Ep {ep} | Recent Avg: {avg:.2f} | Best Avg: {best_moving_avg:.2f}")
    ...
    best_model = GNNActorCritic(node_dim=3, hidden_dim=128, num_actions=24)
